...
| **security-scan** | Drives the published `leverj/security-scan` Docker image (deterministic lanes: CVEs, secrets, SAST patterns, IaC misconfigs, image-CVE, live Supabase) **plus** a host-side companion tool [`security-scan-llm`](tools/security-scan-llm/) (LLM SAST via codex + gemma with bidirectional cross-validation). Both substrates file into the same GitHub Projects v2 board with a shared fingerprint scheme. Each run checks Docker Hub for a newer image and offers a user-confirmed upgrade + config-migration flow, driven by the image's baked-in `SECURITY-SCAN-MANIFEST.yaml`. | [SKILL.md](skills/security-scan/SKILL.md) |
